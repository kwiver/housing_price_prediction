...
def get_filtered_options(df, state=None, region_name=None, furnishing=None):
    ...
    options ={
        ...
        "price_m2s": sorted(filtered_df["price_m2"].unique().tolist()),
...
def predict_price(house_data, pipeline):
    try:
        input_df = pd.DataFrame([house_data])
        predicted_price = pipeline.predict(input_df)[0]

        margin_percentage = 0.15
        min_price = predicted_price * (1 - margin_percentage)
        max_price = predicted_price * (1 + margin_percentage)
...
def main():
    ...
    if st.button("GET RESULT"):
        if not all([state, region_name, price_m2, bathrooms, furnishing]):
            ...
            with st. spinner("Calculating predicted price..."):
                result = predict_price(house_data, pipeline)
    ...
    if st.session_state.show_result and "result" in st.session_state:
        ...
        st.write(
            f"**Price range** ₦{result['min_predicted_price']:,.0f} - "
            f"₦{result['max_predicted_price']:,.0f}"
        )
        
        st.write(
            f"**Furnishing :** {house_data['furnishing']} | "
            f"**Bathrooms :** {house_data['bathrooms']}"
        )
